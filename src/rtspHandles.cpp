...
void RTSPServer::handleTeardown(RTSP_Session& session) {
  RTSP_LOGD(LOG_TAG, "Starting handleTeardown for session %lu", session.sessionID);
  if (!session.isPlaying) {
    RTSP_LOGD(LOG_TAG, "Session %lu already torn down, skipping", session.sessionID);
    return;
  }

  session.isPlaying = false;

  int sock = session.isHttp ? session.httpSock : session.sock;
  if (sock >= 0) {
    char response[128];
    int len = snprintf(response, sizeof(response),
                       "RTSP/1.0 200 OK\r\nCSeq: %d\r\nSession: %lu\r\n\r\n",
    ...
    if (write(sock, response, len) < 0) {
      RTSP_LOGE(LOG_TAG, "Failed to send TEARDOWN response for session %lu", session.sessionID);
    }
  }
  ...
  if (xSemaphoreTake(sessionsMutex, portMAX_DELAY) == pdTRUE) {
    ...
    if (session.isHttp && session.sessionCookie[0] != '\0') {
      ...
      if (pairedSession && pairedSession != &session && sessions.find(pairedSession->sessionID) != sessions.end()) {
        RTSP_LOGD(LOG_TAG, "Found paired HTTP session %lu, marking for cleanup", pairedSession->sessionID);
        pairedSession->isPlaying = false;
        pairedSession->sock = -1;
        pairedSession->httpSock = -1;
        sessions[pairedSession->sessionID] = *pairedSession;
      }
    }
    RTSP_LOGD(LOG_TAG, "Session %lu updated, sessions remaining: %d", session.sessionID, sessions.size());
    xSemaphoreGive(sessionsMutex);
  } else {
  ...
  updateIsPlayingStatus();
  RTSP_LOGD(LOG_TAG, "RTSP Session %lu has been torn down", session.sessionID);
}
...
bool RTSPServer::handleRTSPRequest(RTSP_Session& session) {
  RTSP_LOGD(LOG_TAG, "Starting handleRTSPRequest for session %lu", session.sessionID);
  ...
  if (totalLen <= 0) {
    ...
    if (err == EWOULDBLOCK || err == EAGAIN) {
      ...
      RTSP_LOGD(LOG_TAG, "Connection reset/closed for session %lu", session.sessionID);
      if (xSemaphoreTake(sessionsMutex, portMAX_DELAY) == pdTRUE) {
        if (session.isPlaying) {
          this->handleTeardown(session);
          RTSP_LOGD(LOG_TAG, "HandleTeardown called for session %lu", session.sessionID);
          if (session.isHttp && session.sessionCookie[0] != '\0') {
            RTSP_Session* pairedSession = findSessionByCookie(session.sessionCookie);
            if (pairedSession && pairedSession != &session && pairedSession->isPlaying) {
              RTSP_LOGD(LOG_TAG, "Found paired HTTP session %lu, handling teardown", pairedSession->sessionID);
  ...
  if (strncmp(buffer, "GET / HTTP/", 10) == 0 && strstr(buffer, "Accept: application/x-rtsp-tunnelled")) {
    ...
    bool result = handleRTSPCommand(buffer, session);
    free(buffer);
    return result;
  }
...
bool RTSPServer::handleRTSPCommand(char* command, RTSP_Session& session) {
  RTSP_LOGD(LOG_TAG, "Starting handleRTSPCommand for session %lu", session.sessionID);
